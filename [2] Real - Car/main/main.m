...
plot(0,0,'.','markersize',25,'MarkerFaceColor','b');
hold on;
text((0+50)/1000,0,'Start');
plot(Data.data(:,4)/1000, Data.data(:,5)/1000,'r','linewidth',2);
plot(Data.data(end,4)/1000, Data.data(end,5)/1000,'s','markersize',8,'MarkerFaceColor','k');
text((Data.data(end,4)-200)/1000, (Data.data(end,5)-40)/1000,'End');
% legend('GPS','RTK');
set(gca, 'FontSize', 16);
ylabel('North (Km)','FontSize', 18, 'Interpreter', 'latex');
xlabel('East (Km)','FontSize', 18, 'Interpreter', 'latex');
axis([-0.5 2.5 -1.1 0.6]);

%% Plot Real Velocity
figure;
plot(Data.data(:,1), Data.data(:,6),'b','linewidth',2);
hold on;
plot(Data.data(:,1), Data.data(:,7),'r-.','linewidth',2);
legend('East','North');
set(gca, 'FontSize', 16);
...
%% Load initialization
sys = LoadScenario_Real_Car(EpisodeLength, Data.data);
x_hat0  = sys.init.x0;
P0      = sys.init.P0;
