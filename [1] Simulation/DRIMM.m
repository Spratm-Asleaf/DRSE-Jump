...
% I am happy to see if anyone can prove this method.
    ...
    % Size of ambiguity set of "mu"; I tried, cannot be too large; Otherwise, cannot converge
...
function P = RobustifyOverP(P)
    ...
    if scenario_index == 3        % model set not complete: using three among five
        theta_j = 1.5;
    elseif scenario_index == 4    % only model uncertainty
        theta_j = 1.2;            % 1.2 is good for Table 4
	elseif scenario_index == 5    % model uncertainty + uncertain TPM
        theta_j = 1.25;
    elseif scenario_index == 6    % no uncertainty
        theta_j = 1.25;
    else
        theta_j = 1.5;            % Not intentionally used because this function is not invoked for "scenario_index == 1 and 2"
    end
